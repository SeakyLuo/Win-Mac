...
class coursePlan(Frame):
    def __init__(self,master):
        ...
        self.boxList=[self.timeBox,self.nameBox,self.locBox,self.instructorBox]
        for i,(box,label) in enumerate(zip(self.boxList,[self.timeLabel,self.nameLabel,self.locLabel,self.instructorLabel])):
        ...
        buttonList=[self.addButton,self.clearButton,self.readButton,self.saveButton,self.colorButton]
        for i,button in enumerate(buttonList):
            self.configureButton(button)
            button.grid(row=i//2,column=9+i%2,sticky=NSEW)
        Label(self,text='Course List',relief=FLAT,bg='DeepSkyBlue',\
              bd=0,width=2*self.labelWidth,height=1).grid(row=len(self.boxList),column=6,columnspan=3,sticky=NSEW)
        sortButton=Button(self,text='Sort',command=self.sort)
        self.configureButton(sortButton)
        sortButton.grid(row=len(self.boxList),column=10)       
        dropAllButton=Button(self,text='Drop All',command=self.dropAll)
        self.configureButton(dropAllButton)
    ...
    def putLabel(self,datetime,name,loc,instructor,mode=0):
        ...
        if mode:
            ...
            self.courseInfo.append((datetime,name,loc,instructor))
    ...
    def add(self,mode=0):
        if len(self.courseInfo)==20:
            ...
            return
        info=datetime,name,loc,instructor=(box.get() for box in self.boxList)
        ## 空instructor可以用T.B.A
        if datetime in ('',self.exampleText) or (datetime,name,loc,instructor) in self.courseInfo:
            return
        if name=='':
            messagebox.showerror('Error','Please input course name!')
            return
        self.putLabel(datetime,name,loc,instructor,mode)
        if self.exception:
            self.exception=False
        else:
            self.clear()
    ...
    def read(self):
        ...
        for datetime,name,loc,instructor in content['courseInfo']:
            self.putLabel(datetime,name,loc,instructor)

    def save(self):
        content={'courseInfo':self.courseInfo,'color':self.colorList[0]}
        ez.fwrite('settings.txt',content)
    ...
    def modify(self,button):
        ...
        if button['text']=='Cancel':
            ...
            for i,box in enumerate(self.boxList):
                box.insert(0,info[i])
            self.courseInfo[index]=('' for box in self.boxList)
            self.addButton['command']=lambda :self.modifyMode(button)
            button['text']='Cancel'

    def modifyMode(self,button):
        info=(box.get() for box in self.boxList)
        self.add(info)
    ...
    def sort(self):
        copy=sorted(self.courseInfo.copy(),key=lambda x:x[1])
        self.dropAll()
        for datetime,name,loc,instructor in copy:
            self.putLabel(datetime,name,loc,instructor)
